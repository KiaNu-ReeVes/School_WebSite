  ...
  connection.query(
    ...
    function (err, resallacc) {
      if (resallacc[0] == undefined) {
        connection.query(
          "INSERT INTO users (nationalID,nationalSerial) VALUES (?,?);",
          [req.body.nationalID, req.body.nationalSerial],
          function (err, rese) {
            if (err)
              return res.render("./auth/register", {
      ...
      if (userinfo == null) {
        ...
        if (userinfo.nationalSerial === req.body.nationalSerial) {
          if (req.body.rememberme) {
            ...
            connection.query(
              `UPDATE users SET remember_token = '${Token}' WHERE nationalID = '${req.body.nationalID}'`
            );
